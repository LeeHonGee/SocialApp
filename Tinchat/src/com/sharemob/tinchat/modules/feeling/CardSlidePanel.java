 ...
 * @author xmuSistone
 */
public class CardSlidePanel extends LinearLayout {
    public  List<CardItemView> viewList = new ArrayList<CardItemView>(); // 存放的是每一层的view，从顶到底
    ...
    private static final float SCALE_STEP = 0.7f; // view叠加缩放的步长
    private static final int MAX_SLIDE_DISTANCE_LINKAGE =300; // 水平距离+垂直距离
    // 超过这个值
    // 则下一层view完成向上一层view的过渡
    private View bottomLayout; // 卡片下边的三个按钮布局

    private int bottomMarginTop = 10;
    private final int yOffsetStep = 90; // view叠加垂直偏移量的步长

    private static final int X_VEL_THRESHOLD = 200;
    private static final int X_DISTANCE_THRESHOLD = 200;
    ...
    private List<FeelingEntity> dataList;
...
//        int num = getChildCount()>this.dataList.size()?getChildCount():this.dataList.size();
...
//            }
    ...
    private void orderViewStack() {
        synchronized (obj1) {
            ...
            // 3. changedView填充新数据
            int newIndex = isShowing + 0;
            if (newIndex < dataList.size()) {
            	FeelingEntity entity=dataList.get(newIndex);
            	changedView.fillData(entity);
            } else {
     ...
     * @param dataList 数据
     */
    public void fillData(List<FeelingEntity> list) {
    	this.dataList=list;
    	for (int i = 0; i <viewList.size(); i++) {
    		FeelingEntity entity=list.get(i);
    		CardItemView itemView = (CardItemView) viewList.get(i);
			itemView.fillData(entity);
